...
class StatisticalSpending : Fragment() {
    ...
    private fun setupViews() {
        ...
        val expenseCategories = categories.filter { it.type == TransactionType.EXPENSE.value }.map { it.title }
        val transactionsByCategoryMap = transactionsByCategory?.groupBy { it.category }
        val listSummary = createSummaryList(transactionsByCategoryMap, expenseCategories)
